...
```{r}
switchtube_tbl3 <- switchtube_tbl2 %>% group_by(event_id) %>% mutate(grouped_key_value_pairs = paste0(key_value_pairs, collapse = ""))
```

De-select the key_value_pairs column
```{r}
switchtube_tbl4 <- switchtube_tbl3 %>% select(-c(key_value_pairs))
```
